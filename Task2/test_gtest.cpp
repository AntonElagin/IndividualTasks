...
int get_comment_array(Comment** return_ptr, int* return_size,
                      const char* file_name) {
  if (*return_ptr) return -1;
  ...
  if (fptr) {
    ...
    for (int i = 0; i < *return_size; ++i) {
      fscanf(fptr, "%u %u %lu\n", &return_ptr[0][i].id, &return_ptr[0][i].count,
             &return_ptr[0][i].mark.status);
    }
    fclose(fptr);
    return 0;
  }
...
TEST(Naive, Empty_array) { ASSERT_EQ(find_zero_count_naive(NULL, -10), 0); }
